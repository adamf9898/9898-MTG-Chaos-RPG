...
class AIService {
            ...
            const specialMechanics = [
                {
                    ...
                    effect: 'Turn order reverses randomly',
                    trigger: 'combat',
                },
                {
                    name: 'Mana Cascade',
                    effect: 'Spells cost 1 less but have random effects',
                    trigger: 'spell_cast',
                },
                {
                    name: 'Reality Shift',
                    effect: 'Creatures swap controller each turn',
                    trigger: 'turn_start',
                },
                {
                    name: 'Chaos Multiplication',
                    effect: 'All numerical values doubled',
                    trigger: 'always',
                },
                {
                    name: 'Planar Resonance',
                    effect: 'Cards in graveyard can be cast',
                    trigger: 'main_phase',
                },
            ];
        ...
        const choices = [
            {
                ...
                options: [
                    {
                        choice: 'Show mercy',
                        consequence: 'Gain favor but lose reward',
                        alignment: 'good',
                    },
                    {
                        choice: 'Complete the contract',
                        consequence: 'Full reward but moral cost',
                        alignment: 'neutral',
                    },
                    {
                        choice: 'Demand double payment',
                        consequence: 'Risk confrontation',
                        alignment: 'chaotic',
                    },
                ],
            },
                ...
                options: [
                    {
                        ...
                        consequence: 'Truth revealed but potential conflict',
                        alignment: 'lawful',
                    },
                    {
                        choice: 'Play along',
                        consequence: 'Maintain relationship but enable deception',
                        alignment: 'neutral',
                    },
                    {
                        choice: 'Turn the tables',
                        consequence: 'Chaotic outcome',
                        alignment: 'chaotic',
                    },
                ],
            },
    ...
    generateQuestGiver(personality) {
        ...
        return questGivers.filter((qg) => qg.trustworthy > 0.7)[Math.floor(Math.random() * 3)];
    }
    ...
    generateBossDialogue(boss, gameState) {
        ...
        return {
            encounterStart:
                dialogues.encounter_start[
                    Math.floor(Math.random() * dialogues.encounter_start.length)
                ],
            halfHealth:
                dialogues.half_health[Math.floor(Math.random() * dialogues.half_health.length)],
